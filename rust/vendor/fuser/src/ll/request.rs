...
mod op {
    ...
    impl<'a> SetAttr<'a> {
        ...
        pub fn atime(&self) -> Option<TimeOrNow> {
            match self.arg.valid & FATTR_ATIME {
                0 => None,
                _ => {
                    if self.arg.atime_now() {
                        Some(TimeOrNow::Now)
                    } else {
                        checked_system_time_from_time(self.arg.atime, self.arg.atimensec)
                            .map(TimeOrNow::SpecificTime)
                    }
                }
            }
        }
        pub fn mtime(&self) -> Option<TimeOrNow> {
            match self.arg.valid & FATTR_MTIME {
                0 => None,
                _ => {
                    if self.arg.mtime_now() {
                        Some(TimeOrNow::Now)
                    } else {
                        checked_system_time_from_time(self.arg.mtime, self.arg.mtimensec)
                            .map(TimeOrNow::SpecificTime)
                    }
                }
            }
        ...
        pub fn ctime(&self) -> Option<SystemTime> {
            ...
            match self.arg.valid & FATTR_CTIME {
                0 => None,
                _ => checked_system_time_from_time(self.arg.ctime, self.arg.ctimensec),
            }
        ...
        pub fn crtime(&self) -> Option<SystemTime> {
            ...
            match self.arg.valid & FATTR_CRTIME {
                0 => None,
                _ => checked_system_time_from_unsigned(self.arg.crtime, self.arg.crtimensec),
            }
        ...
        pub fn chgtime(&self) -> Option<SystemTime> {
            ...
            match self.arg.valid & FATTR_CHGTIME {
                0 => None,
                _ => checked_system_time_from_unsigned(self.arg.chgtime, self.arg.chgtimensec),
            }
            #[cfg(not(target_os = "macos"))]
            None
        }
    ...
    pub(super) fn checked_system_time_from_unsigned(secs: u64, nsecs: u32) -> Option<SystemTime> {
        SystemTime::UNIX_EPOCH.checked_add(Duration::new(secs, nsecs))
    }

    fn checked_system_time_from_time(secs: i64, nsecs: u32) -> Option<SystemTime> {
        if secs >= 0 {
            checked_system_time_from_unsigned(secs as u64, nsecs)
        } else {
            SystemTime::UNIX_EPOCH.checked_sub(Duration::new((-secs) as u64, nsecs))
        }
    }
...
mod tests {
    ...
    #[test]
    fn checked_system_time_from_unsigned_returns_none_on_overflow() {
        let overflow_secs = u64::MAX;
        let result = op::checked_system_time_from_unsigned(overflow_secs, 0);

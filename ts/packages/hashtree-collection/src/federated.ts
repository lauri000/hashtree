...
export async function federatedSearch(
  store: Store,
  sources: Iterable<FederatedCollectionSource>,
  indexName: string,
  query: string,
  ...
  const localResults = await Promise.all(
    sourceList.map(async (sourceInput) => {
  ...
  const merged = new Map<string, FederatedSearchHit & { bestWeightedScore: number }>();

  for (const resultSet of localResults) {
    for (const result of resultSet) {
      const hit: FederatedSearchSourceHit = {

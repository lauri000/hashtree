...
async function main() {
  ...
  try {
    ...
    const hasSearch = await page.getByPlaceholder('Search videos').isVisible().catch(() => false);
    const hasLogin = await page.getByRole('button', { name: /^Login$/ }).isVisible().catch(() => false);
    if (!hasSearch && !hasLogin) {
      const bodyPreview = (await page.locator('body').innerText()).slice(0, 500);
      throw new Error(`Portable build did not render the video shell. Body preview: ${bodyPreview}`);
    }
    ...
    console.log(`Portable Iris video smoke passed: ${url}`);

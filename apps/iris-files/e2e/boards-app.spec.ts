...
test.describe('Iris Boards App', () => {
  ...
  test('trello-like cards use modal editing and can be dragged between columns', async ({ page }) => {
    ...
    await createdCard.getByRole('button', { name: /attach file/i }).click();
    ...
    await expect(createdCard.getByText('notes.txt')).toBeVisible({ timeout: 10000 });
    const attachmentImage = createdCard.getByRole('img', { name: 'tiny.png' });
    await expect(attachmentImage).toBeVisible({ timeout: 10000 });
    await expect.poll(async () => {
      return attachmentImage.evaluate((img: HTMLImageElement) => img.complete && img.naturalWidth > 0);
    }, { timeout: 20000 }).toBe(true);
    const imageSrc = await attachmentImage.getAttribute('src');
    expect(imageSrc).toContain('/htree/nhash1');
    await expect(createdCard.getByText(/^Uploaded$/)).toHaveCount(0);
    ...
    await mediaDialog.getByRole('button', { name: /close attachment preview/i }).click();
    await expect(mediaDialog).toHaveCount(0);

    await createdCard.getByRole('button', { name: /edit card/i }).click();
  ...
  test('link-visible board syncs to another browser in realtime without reload', async ({ page, browser }) => {
    ...
    const page1Pubkey = await page.evaluate(() => (window as any).__nostrStore?.getState?.().pubkey ?? null);
    const initialPage2Pubkey = await page2.evaluate(() => (window as any).__nostrStore?.getState?.().pubkey ?? null);
    ...
    const liveMarker = await page2.evaluate(() => {
      ...
      (window as any).__boardLiveMarker = marker;
      return marker;
    });
    ...
    await page.getByTestId('board-card-Realtime card').getByRole('button', { name: /edit card/i }).click();
    await expect(page.getByRole('heading', { name: 'Edit Card' })).toBeVisible({ timeout: 10000 });
    ...
    await expect.poll(async () => {
      return page2.evaluate(() => (window as any).__boardLiveMarker);

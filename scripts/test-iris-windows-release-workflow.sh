...
grep -F "Copy-Item \$nsis.FullName \"dist/iris-\$env:RELEASE_TAG-windows-x64-setup.exe\"" .github/workflows/release.yml >/dev/null
grep -F "Run \`iris-\${{ env.RELEASE_TAG }}-windows-x64-setup.exe\`." .github/workflows/release.yml >/dev/null
